...
var swiper_promoted = new Swiper('.swiper__promoted-container', {
  slidesPerView: 4,
  spaceBetween: 30,
  slidesPerGroup: 4,
  loop: true,
...
var slides = 0;
var swiper_category = new Swiper('.swiper__category-container', {
  slidesPerView: 4,
  slidesPerColumn: 2,
  slidesPerGroup: 4,
  spaceBetween: 30,
  ...
  on: {
    init: function init() {
      ...
      var numbersBlank = 8 - slides % 8;
      if (slides % 8 != 0) {
        for (var i = 0; i < numbersBlank; i++) {
          this.appendSlide(['<div class="swiper-slide category swiper-slide--blank"></div>']);
        }
      }
    }
  }
...
var couponOfferImg = $('.coupon__offer-img');
couponOfferImg.matchHeight({
  byRow: 0
});

var brandCard = $('.brand__element');
...
var brandFavorite = $('.brand__favorite');
...
/* STAR BRAND */
$('.brand__halfstar').hover(function () {
  var starRating = $(this).data('score');
  //let starVote = $(this).parent();
  var starVote = $(this).closest('.brand__stars');
...
accordion.on("click", function () {
  if ($(this).parent().find('.accordion__number').hasClass('active-colors')) {
    ...
    var heightAnswer = $(this).parent().find('.panel__answer').height();
    allPanels.slideUp();
    $(".accordion__number").removeClass('active-colors');
    $(this).next().slideDown();
